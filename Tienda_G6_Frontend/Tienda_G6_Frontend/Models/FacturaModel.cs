...
namespace Tienda_G6_Frontend.Models
{
    public class FacturaModel
    {
        ...
        public List<FacturaEnt> ConsultarFacturas()
        {
        ...
        public RespuestaApi ConsultarFactura(long q)
        {
            using (var client = new HttpClient())
            {
        ...
        public RespuestaApi RegistrarFactura(FacturaEnt entidad)
        {
            using (var client = new HttpClient())
            {
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/AgregarFactura";
                string token = HttpContext.Current.Session["TokenUsuario"].ToString();
        ...
        public RespuestaApi ActualizarFactura(FacturaEnt entidad)
        {
            using (var client = new HttpClient())
            {
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/ActualizarFactura";
                string token = HttpContext.Current.Session["TokenUsuario"].ToString();
                JsonContent body = JsonContent.Create(entidad);
        ...
        public RespuestaApi EliminarFactura(long q)
        {
            using (var client = new HttpClient())
            {
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/EliminarFactura?q=" + q;
                string token = HttpContext.Current.Session["TokenUsuario"].ToString();
