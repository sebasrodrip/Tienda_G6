...
namespace Tienda_G6_Frontend.Models
{
    public class BitacoraModel
    {
        ...
        public List<BitacoraEnt> ConsultarBitacoras()
        {
            using (var client = new HttpClient())
            {
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/ConsultarBitacoras";
                ...
                {
                    return resp.Content.ReadFromJsonAsync<List<BitacoraEnt>>().Result;
                }

                return new List<BitacoraEnt>();
            }
        }

        public RespuestaApi ConsultarBitacora(long q)
        {
            using (var client = new HttpClient())
            {
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/ConsultarBitacora?q=" + q;
                string token = HttpContext.Current.Session["TokenUsuario"].ToString();
